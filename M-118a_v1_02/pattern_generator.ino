...
/* static */
uint8_t PatternGenerator::count_;
...
void PatternGenerator::EvaluateDrums() {
  ...
  if (step_ == 0) {
    for (uint8_t i = 0; i < kNumParts; ++i) {
      uint8_t randomness = options_.swing ? 0 : settings_.options.drums.randomness >> 2;
  ...
  if (output_clock()) {
    ...
    // state_ |= step_ == 0 ? OUTPUT_BIT_RESET : 0; //NOT USED AT THE MOMENT
  } else {
    state_ |= accent_bits << 2;//SHOULD BE 2 and not 3 Cause only use 2 instruments cause of the new structure pattern---------------------------E-LICK
  }
}
...
void PatternGenerator::EvaluateEuclidean() {
  ...
  if (output_clock()) {
    ...
    state_ |= reset_bits << 2;//SHOULD BE 2 and not 3 Cause only use 2 instruments  cause of the new structure pattern---------------------------E-LICK
  }
}
...
void PatternGenerator::LoadSettings() {
  ...
  clk_out_div = eeprom_read_byte((uint8_t*)(2));
  if (clk_out_div > 3) clk_out_div = 3;
}
...
void PatternGenerator::SaveSettings() {
  ...
  eeprom_write_byte((uint8_t*)(2), clk_out_div);
}
...
void PatternGenerator::Evaluate() {
  ...
  /*Random::Update();
    state_ |= Random::state() & 0x80;//NOT USED AT THE MOMENT
  */

  // Highest bits: clock and random bit.
  //This code allows user to divide input clock by 2 or 4 when in slave clock mode
byte div_value[] = {1, 2, 4, 8};
  
  if ( count_ == div_value[clk_out_div]) {
    state_ |= OUTPUT_BIT_CLOCK;//Clock output is BIT4------------------------------------------------------------ELICK
    count_ = 0;
  }
  if (count_ > 8)  count_ = 0;
