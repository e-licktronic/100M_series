...
//                                MODDED ELICK
// OUTPUT MODE  OUTPUT CLOCK  BIT7  BIT6  BIT5  BIT4  BIT3  BIT2  BIT1  BIT0
// DRUMS        FALSE          RND   CLK  HHAC        SDAC  BDAC    SD    BD
// DRUMS        TRUE           RND   CLK   CLK   BAR   ACC    HH    SD    BD ???
// EUCLIDEAN    FALSE          RND   CLK  RST3  RST2  RST1  EUC3  EUC2  EUC1 ???
// EUCLIDEAN    TRUE           RND   CLK   CLK  STEP   RST  EUC3  EUC2  EUC1 ???
...
const uint8_t kNumParts = 2;//only two instrument not 3--------------------------------------E-LICK
...
enum OutputBits {
  OUTPUT_BIT_COMMON = 0x08,//BIT4
  OUTPUT_BIT_CLOCK = 0x10,//BIT5
  OUTPUT_BIT_RESET = 0x20//BIT6
};
...
class PatternGenerator {
  public:
    ...
    static inline uint8_t step() {
      return step_;
    }

    static inline bool swing() {
      return options_.swing;
    }
    static int8_t swing_amount();
    static inline bool output_clock() {
      return options_.output_clock;
    }
    static inline bool tap_tempo() {
      return options_.tap_tempo;
    }
    static inline bool gate_mode() {
      return options_.gate_mode;
    }
    static inline OutputMode output_mode() {
    ...
    static void set_swing(uint8_t value) {
      options_.swing = value;
    }
    static void set_output_clock(uint8_t value) {
      options_.output_clock = value;
    }
    static void set_tap_tempo(uint8_t value) {
      options_.tap_tempo = value;
    }
    ...
    static inline void IncrementPulseCounter() {
      ...
      if (pulse_duration_counter_ >= kPulseDuration && !options_.gate_mode) {
        state_ &= OUTPUT_BIT_CLOCK;//reset trig out but clock
    ...
    static bool on_first_beat() {
      return first_beat_;
    }
    static bool on_beat() {
      return beat_;
    }
    static bool factory_testing() {
      return factory_testing_ < 5;
    }

    static void SaveSettings();

    //TRIG LED ACCORDING THE  DRUM PATTERN
